...
**
** Summary: C++ class that handles QML code for Fridge Contents controls
**
****************************************************************************/


#include "fridgecontents.h"
#include "ui_fridgecontents.h"
#include "CustomWidgets/Fridge-Contents/xmlparser.h"
#include <QDeclarativeContext>
#include <QDir>

/***
 ** Constructor
 ** Create a QDeclarativeView object and load the .qml
 ** Add the new widget to the UI layout (left hand side)
 **/
FridgeContents::FridgeContents(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FridgeContents)
{
    ui->setupUi(this);
    ...
    fileName = path + "/webpage/fridge_contents.xml";
    ...
    qmlView->rootContext()->setContextProperty("cppXMLParser", xmlParser);   // pass an address of xmlParser to QML
    qmlView->rootContext()->setContextProperty("fileName", fileName); // pass the XML file name to QML


    if (isSmallResolution) {
        this->setGeometry(0, 0, 480, 272);
        qmlView->setSource(QUrl("qrc:/qml/smallFridgeContents/main.qml"));
    } else {
        qmlView->setSource(QUrl("qrc:/qml/FridgeContents/main.qml"));
    }

    ui->contentsLayout->addWidget(qmlView);
}
