...
**
** Summary: Overlay button element.
**
****************************************************************************/
...
Item {
    ...
    objectName: "overlayObject"
    property bool on: false

    // Based on the button clicked, send appropriate signal
    function applicationClicked() {

        gridViewOverlay.resetOverlayButtonStates()

        if(applicationName == "calendar"){
            overlay.onCalendarButtonPressed()
        }else if(applicationName === "contents"){
            overlay.onContentsButtonPressed()
        }else if(applicationName === "epi"){
            overlay.onEpiButtonPressed()
        }else if (applicationName === "internet"){
            overlay.onInternetButtonPressed()
        }else if (applicationName === "whiteboard"){
            overlay.onWhiteboardButtonPressed()
        }else if (applicationName === "zones"){
            overlay.onZonesButtonPressed()
        }else if (applicationName === "contents"){
            overlay.onContentsButtonPressed()
        }
    ...
    function toggle() {
        ...
        if (applicationItem.state == "On"){
            applicationItem.state = "Off";

        }else{
            applicationItem.state = "On";
        }

    }

    // Buttons have on and off state. Appropriate images are used for this functionality
    Item {
        width: 62; height: 44; anchors.left: parent.left

        Column {
            spacing: 5
            Rectangle {
                id: appRect; width: 62; height: 44; color: "#00000000"; smooth: false
                Image { id: offButton;
                    source: applicationImageOff;
                    x: 1; y: 1; width: 49; height: 44;
                    smooth: false ;
                    anchors.left: parent.left
                    opacity: on ? 0: 1
                }
                Image { id: onButton;
                    source: applicationImageOn;
                    x: 1; y: 1; width: 62; height: 44;
                    smooth: false ;
                    anchors.left: parent.left
                    opacity: on ? 1: 0
                }
    ...
    states: [
        State {
             name: "Off";
             PropertyChanges { target: applicationItem; on: false }
         },
         State {
             name: "On"
             PropertyChanges { target: applicationItem; on: true }
     ...
     transitions: [
         Transition {
             from: "Off"; to: "On"
             SequentialAnimation {
                 NumberAnimation { target: applicationItem; property: "scale"; to: 0.8; duration: 250 }
                 NumberAnimation { target: applicationItem; property: "scale"; to: 1; duration: 250 }
             }
         }
     ]

    MouseArea { anchors.fill: applicationItem; onClicked: {
                                                            applicationClicked()
                                                            toggle()
