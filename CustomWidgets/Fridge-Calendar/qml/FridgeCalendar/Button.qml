...
**
** Summary: Implements all logic for a generic button
**
**
****************************************************************************/
...
Rectangle {
    ...
    property string buttonImageFront
    property string buttonImageBack
    property bool dualIcon: false

    //signal to be generated when the button is clicked
    signal buttonClicked()

    //Image to be placed on the button. Two images are allowed to represent button states
    Image {
        id: genericButton
        anchors.centerIn: parent
        source: !dualIcon ? buttonImageFront : buttonImageBack
    }
    ...
    //generate a buttonClicked signal upon mouse press
    Component.onCompleted: {
        mousearea.clicked.connect(buttonClicked)
    }

    //Timer for the animation - upon expiration, changes button state to released
    Timer {
        id: stateTimer
        interval: 1;
        repeat: false
    ...
    //Animation for transition between the "states" of the button. Animated property is "scale"
    transitions: Transition {
        SequentialAnimation{
            NumberAnimation { target: button; property: "scale"; to: 1.0; duration: 10 }
            NumberAnimation { target: button; property: "scale"; to: 1.2; duration: 200 }
            NumberAnimation { target: button; property: "scale"; to: 1.0; duration: 200 }
        }
    }
}
