...
**
** Summary: Implements Day View for the Calendar Screen. Day View
** includes a list of events for a day. This implementation does not store events when
** switching days. Helper functions to manage events are provided.
**
****************************************************************************/
...
Item
{
    id: mainCalendarDayRect

    //Helper function to set text for a given event
    function setTextForEvent(txt)
    {
        for(var i=0; i< dayEventList.children.length; i++)
        {
            var item = dayEventList.children[i];
            if (item.objectName === "dayEvent")
            {
                if(item.mSelected){
                    item.mSelected = false
                    item.eventTextString = txt
                }
            }
        }
    }

    // Removes all events from the DayView
    ...
    //We want the day view to be flickable so that it can be moved with a finger
    Flickable
    {
        id: container
        anchors.fill: parent
        contentWidth: mainCalendarDayRect.width
        contentHeight: mainCalendarRect.dayHeight
        flickableDirection: Flickable.VerticalFlick

        Row
        {
            // Generate Hour scale for the day
            DayTimes
            {
                id: dayT
                width: 50
                height: mainCalendarRect.dayHeight
            }

            // Initiate Calendar Event List. Once clicked an event will be added to a day view
            CalendarDayEventlist
            {
                id: dayEventList

                rootElement: container
                width: mainCalendarDayRect.width - dayT.width
                height: mainCalendarRect.dayHeight
            }
        }
    }
