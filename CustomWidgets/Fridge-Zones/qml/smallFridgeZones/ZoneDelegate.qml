...
**
** Summary: Every Zone has a temperature parameter
**
****************************************************************************/
...
Item {
    id: zoneSwitch; width: GridView.view.cellWidth-1; height: GridView.view.cellHeight-1

    objectName: "zoneObject"
    property bool on: false
    property bool tempChanged: false

    // When a zone is clicked
    function zoneClicked() {
        dial.setSlider((parseInt(tempTect.text)+5)/0.6)
        gridViewZones.resetButtonStates()
    }

    // Show temp dial on toggle
    function toggle() {

        if (zoneSwitch.state == "On"){
            zoneSwitch.state = "Off";

        }else{

            // set dial pos, base on current item pos - 0 move to the ridht side, otherwise to left
            if (zoneSwitch.x != 0) {
                dial.isLeft = 2;
            } else {
                dial.isLeft = 1;
    ...
    Item {
        ...
        // Show this image when zone in the "off" state
        BorderImage {
            id: borderOff
            anchors.fill: parent;
            source: imageOff;
            opacity:{ if(on) { return 0} else { if(tempChanged) {return 0.3 } else {return 1.0}}}

        }

        // Show this image when zone in the "on" state
        BorderImage {
            anchors.fill: parent;
            source: imageOn;
            opacity: on ? 1: 0
        ...
        // Temp text to be displayed on each zone
        Text {
            id: tempTect;
            anchors.centerIn: parent
            text: name === "VEGETABLES" || name === "MAIN"? temp+"\u00B0" + " " : " " + temp+"\u00B0";
            color: on? "white": "#464646"
            font.pointSize: 90;
            Behavior on text {
                 SequentialAnimation {
                     NumberAnimation { target: tempTect; property: "opacity"; to: 1; duration: 550 }
                     NumberAnimation { target: tempTect; property: "opacity"; to: 0; duration: 550 }
                     NumberAnimation { target: tempTect; property: "opacity"; to: 1; duration: 550 }
                 }
            }
        }
   ...
   states: [
       State {
            name: "Off";
            PropertyChanges { target: zoneSwitch; on: false }
        },
        State {
            name: "On"
            PropertyChanges { target: zoneSwitch; on: true }
        }
    ]
    transitions: [
        Transition {
            from: "Off"; to: "On"
            SequentialAnimation {
                NumberAnimation { target: zoneSwitch; property: "scale"; to: 0.8; duration: 250 }
                NumberAnimation { target: zoneSwitch; property: "scale"; to: 1; duration: 250 }
            }
        }
    ]
