...
**
** Summary: Temperature dial element
**
****************************************************************************/
...
Item {
    id: tempDial
    property real value: (tempSlider.x * 100 / (sliderRegion.width - 27))
    ...
    Image {
        id: baseImage;
        x:-20; y:-18;
        source: "../images/temp-dial/zones-button-base.png"
        ...
        MouseArea {
            ...
            onClicked:
            {
                gridViewZones.setTemperature()
                stateTimer.restart()
            }
        }
    }
    Image { x:0; y:2; source: "../images/temp-dial/zones-button-dial.png" }

    // Pointer that travels around the dial
    Image {
        id: pointer
        x: 80; y: -16
        ...
        transform: Rotation {
            id: pointerRotation
            origin.x: 4; origin.y: 98

            angle: Math.min(Math.max(-130, tempDial.value*2.6 - 130), 133)
            Behavior on angle {
                SpringAnimation {
                    spring: 1.4
                    damping: .15
    ...
    // Button to increase the temp by 1 degree
/*    Image {
        id: imagePlus;
        x:176; y:48;
        width: 64; height: 64;
        source: "../images/temp-dial/zones-increase.png"

        MouseArea {
            id: plusMouseArea
            anchors.fill: parent
            onClicked: { setSlider(value+1)
                         stateTimer.restart()
                         gridViewZones.setTemperature() }
        }
        ...
        states: State {
                    name: "pressed"; when: plusMouseArea.pressed
                    PropertyChanges { target: imagePlus; scale: 1.2 }
                }
    ...
    // Button to decrease the temp by 1 degree
    Image {
        id: imageMinus;
        x:-72; y:48;
        width: 64; height: 64;
        source: "../images/temp-dial/zones-decrease.png"
        scale: 1.0
        ...
        states: State {
                    name: "pressed"; when: minusMouseArea.pressed
                    PropertyChanges { target: imageMinus; scale: 1.2 }
                }
    ...
    // Slider that's inside the Temperature Dial
    Image {
        id: sliderRegion;

        anchors { bottom: parent.bottom; left: parent.left
            right: parent.right; leftMargin: 16; rightMargin: 16
            bottomMargin: 60 }

        source: "../images/temp-dial/zones-slider-track.png"
        scale: 1.0
        ...
        Image {
            ...
            source: "../images/temp-dial/zones-slider.png"
            scale: 1.0

            MouseArea {
                id: mousearea1
                anchors.fill: parent
                anchors.margins: -16 // Increase mouse area a lot outside the tempSlider
                drag.target: parent; drag.axis: Drag.XAxis
                drag.minimumX: 2; drag.maximumX: sliderRegion.width - 25
                onClicked: stateTimer.restart()
                onPositionChanged: { stateTimer.restart()
                    gridViewZones.setTemperature()
                }
            }
        }
    ...
    onIsLeftChanged: {
        if (isLeft == 1) {
            tempDial.state = "sRight";
        } else {
            tempDial.state = "sLeft";
        }
    }
    ...
    states: [
       State {
        ...
        State {
            name: "Hide"
            PropertyChanges { target: tempDial; scale: 0.0 }
        },
    ...
    transitions: [
        Transition {
            ...
            AnchorAnimation { duration: 500; easing.type: Easing.OutBack }
        }
    ...
    {
        tempSlider.x = val * (sliderRegion.width - 27)/100
        if(tempSlider.x<2)
            tempSlider.x = 2
        if(tempSlider.x> (sliderRegion.width - 25))
            tempSlider.x = sliderRegion.width - 25
    }
}
