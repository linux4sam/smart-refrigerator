...
**
** Summary: Main QML file for Fridge Zones screen
**
****************************************************************************/
...
Rectangle {
    id: fridgeZonesRectangle
    width: 480
    height: 272
    color: "#464646"

    property bool farenheit: true

    // Model for Fridge Zones
    ListModel {
        id: fridgeModel

        property alias fridgeModel : fridgeModel

        ListElement {
            name: "MAIN"
            temp: "12"
            imageOn: "images/mainOn.png"
            imageOff: "images/mainOff.png"
        }
        ListElement {
            name: "FRUIT"
            temp: "16"
            imageOn: "images/fruitOn.png"
            imageOff: "images/fruitOff.png"
        }
        ListElement {
            name: "VEGETABLES"
            temp: "-3"
            imageOn: "images/veggieOn.png"
            imageOff: "images/veggieOff.png"
        }
        ListElement {
            name: "FREEZER"
            temp: "8"
            imageOn: "images/freezerOn.png"
            imageOff: "images/freezerOff.png"
        }
    }

    // All zones are layed out in a grid
    GridView {
        id: gridViewZones
        x: 0
        y: 0
        ...
        // Zone Delegate provides implementation for each zone
        ...
        function resetButtonStates()
        {
            for(var i=0; i< contentItem.children.length; i++)
            {
                ...
                if (item.objectName === "zoneObject")
                {
                    item.on = false
                    item.state = "Off"
                    item.tempChanged = false
                }
            }
        }
        ...
        function setTemperature()
        {
            for(var i=0; i< contentItem.children.length; i++)
            {
                ...
                if (item.objectName === "zoneObject")
                {
                    if(item.state === "On")
                    {
                        fridgeModel.setProperty(i-1, "temp", getTemp())
                    }
        ...
        {
            if(!farenheit)
                 return Math.round(((dial.value*0.6)-5-32)/1.8)
            else
                return Math.round((dial.value*0.6)-5)
        }
    }
    ...
    Dial {
        id: dial
        x: width * .5 + dial.width * .5
        anchors.verticalCenter: parent.verticalCenter
    }

    // Hide temperature dial if no activity for 5 seconds
    Timer {
        id: stateTimer
        interval: 5000;
        running: true
