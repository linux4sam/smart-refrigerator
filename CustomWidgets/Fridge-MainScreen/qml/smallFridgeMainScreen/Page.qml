...
Item {
    ...
    function settingsClicked() {
        // TODO: place to implement settings screen trigger
    }
    ...
    FolderListModel {
            id: folderModel
            nameFilters: ["*.jpg"]
            folder: "../../smallResources"
    }

    // Rotate through images in the directory
    Repeater {
        id: repeaterImg;
        model: folderModel;
        delegate: Image {
    ...
    Image {
        ...
        source: "../../smallResources/settings.png"
    }

    // white rectangle behind buttons
    Rectangle {
    ...
    // Load application names from external file
    RssModel
    {
        id: innerModel
    }
    ...
    Text {
        id: timeText
        anchors { right: parent.right; rightMargin: 12; top: parent.top; topMargin: 6 }
        color: "#FFFFFF"
        font.pointSize: 13
        font.bold: true
    }
    ...
    Text {
        ...
        color: "#FFFFFF"
        font.pointSize: 11
        text: "Loading Screens... Please Wait"
        visible: loading
    }
    ...
    Timer {  //settings Button animation timer
        id: stTimer
        interval: 200;
        repeat: false
        onTriggered: page.state = "settingsDefault"
    }

    Timer {  //background switching timer
        id: backgroundTImer;
        interval: 5500; // delay between background switches
        running: true;
        repeat: true;
        onTriggered: {
            if (currentImage < folderModel.count -1) {
                currentImage++;
            }
            else {
                currentImage = 0; // start from the beginning
            }
        }
    }

    MouseArea { anchors.fill: settingsButton; onClicked: {
            page.state = "settingsPressed"
            stTimer.start()
            settingsClicked()
        }
    }
    ...
    states: [
       State {
            ...
            PropertyChanges { target: settingsButton; scale: 1 }
        },
        State {
            name: "settingsPressed"
            PropertyChanges { target: settingsButton; scale: 1.3 }
        }
    ]
    transitions: [
        Transition {
