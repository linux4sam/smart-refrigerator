...
**
** Summary: Food picker element allows users to select food category by rotating a dial of the widget
**
****************************************************************************/
...
Rectangle {
    ...
    property variant currentFoodSelection: foodDial.currentValue
    property string url: ""
    property alias backgroundImagePath: backgroundImage.source

    signal catSel(string category)

    // Based on selection open appropriate URL in a web browser
    // First, count down from 3 after selection is made to give user a chance to change food category
    function categorySelected(catValue) {
        countTimer.stop()
        ...
        switch(catValue){
            ...
            case "TOP-RATED":
                url = "http://www.epicurious.com/articlesguides/bestof/toprecipes/"
                countTimer.start()
            break;
            case "HOLIDAYS":
                url = "http://www.epicurious.com/recipesmenus/holidays"
                countTimer.start()
            break;
            case "QUICK&EASY":
                url = "http://www.epicurious.com/recipesmenus/quickeasy/recipes"
                countTimer.start()
            break;
            case "HEALTHY":
                url = "http://www.epicurious.com/recipesmenus/healthy/recipes"
                countTimer.start()
            break;
            case "SEASONAL":
                url = "http://www.epicurious.com/recipesmenus/seasonal/recipes"
                countTimer.start()
            break;
            case "DESSERT":
                url = "http://www.epicurious.com/recipesmenus/desserts"
                countTimer.start()
            break;
            case "INTERNATIONAL":
                url = "http://www.epicurious.com/recipesmenus/global/recipes"
                countTimer.start()
            break;
            case "DRINKS":
                url = "http://www.epicurious.com/recipesmenus/drinks"
                countTimer.start()
            break;
            default:
                url = ""
    ...
    // Activate url in a web browser
    function gotoEpicurious()
    {
        if(!countTimer.running & url != "")
            catSel(url)
    }

    onCurrentFoodSelectionChanged: {
        categorySelected(foodDial.currentValue)
    }

    // Text on the top of the widget
    Text {
        id: ttext
    ...
    // Hide widget when user clicks done
    Image {
        id: done
        anchors.right: parent.right
        anchors.rightMargin: 5

        source: "pics/button-done.png"
        ...
        MouseArea {
            anchors.fill: parent
            onClicked: {
                foodPicker.state = "hideFoodPicker";

//                PropertyAnimation { target: foodPicker; properties: "scale"; to: 0; duration: 200; }
            }
        }
    }
    ...
    states: [
        State {
            name: "showFoodPicker";
            PropertyChanges { target: foodCategory; scale: 1; z: 1 }
            PropertyChanges { target: bgShade; opacity: 0.5 }
        },
        State {
            name: "hideFoodPicker";
            PropertyChanges { target: foodCategory; scale: 0 }
            PropertyChanges { target: bgShade; opacity: 0. }
        }
    ]

    transitions: Transition {
        PropertyAnimation { properties: "scale"; duration: 200; }
        PropertyAnimation { properties: "opacity"; duration: 200; }
    }

    // Dial image on which we overlay PathView with food categories
    Image {
        id: backgroundImage
        anchors {top:ttext.bottom; topMargin: 20; horizontalCenter: parent.horizontalCenter }
        source: "pics/food-picker.png"
        ...
        Item {
            ...
            FoodPathView {
                id: foodDial
                width: 200
                model: ["TOP-RATED", "HOLIDAYS", "SELECT", "QUICK&EASY", "HEALTHY", "SEASONAL", "DESSERT", "INTERNATIONAL", "DRINKS"]
            }
        }
    }

    // Overlay an image on the top for improved look
    Image {
        id: overlayImage
        anchors {verticalCenter: backgroundImage.verticalCenter; horizontalCenter: backgroundImage.horizontalCenter }
        source: "pics/food-picker-overlay.png"
    }

    // Counter that allows users to change selection before switching to a web browser. Default is set to 3s
    CountDownBusy {
        id: countDown
        anchors {top:backgroundImage.bottom; topMargin: 5; horizontalCenter: parent.horizontalCenter }
        ...
        Timer {
            ...
            repeat: true
            onTriggered: {
                countDown.setText(countDown.counter)
