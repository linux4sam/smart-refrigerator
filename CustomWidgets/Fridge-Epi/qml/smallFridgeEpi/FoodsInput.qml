...
**
** Summary: Text Input field that allows users to enter foods using the onscreen keyboard
**
****************************************************************************/
...
Item {
    ...
    property alias image: inputTextImage.source
    property alias url: foodText.text

    signal foodEntered(string food)

    width: parent.height; height: parent.height

    // image for text entry
    BorderImage {
        id: inputTextImage; rotation: 180
        x: 4; width: parent.width - 8; height: 20;
        anchors.verticalCenter: parent.verticalCenter
        border { left: 5; top: 5; right: 5; bottom: 5 }
    }

    // element that will receive text entry from the user
    TextInput {
        id: foodText
        ...
        MouseArea {
                 ...
                 onClicked: {
                     ...
                     if (!foodText.activeFocus) {
                         foodText.forceActiveFocus();

                     } else {
                         foodText.focus = false;
                     }
                 }
                 onPressAndHold: keyboard.visible = false;
        }
        anchors {
            left: parent.left; right: parent.right; leftMargin: 9; rightMargin: 9
    ...
    Keyboard {
        ...
        verticalCenter: parent.verticalCenter; verticalCenterOffset: -105}
        visible: false
    }
