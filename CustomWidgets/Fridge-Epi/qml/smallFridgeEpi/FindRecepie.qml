...
**
** Summary: Flippable element with a "Find Recepie" button on one side and a "Web Browser" element on
**          the other.
**
****************************************************************************/
...
Rectangle {
    ...
    //properties used in this QML file and throughout the Epicurious implementation
    property alias webBrowser: webBrowserContainer
    property alias stackDeleg: recepieDelegate
    property int index: 0
    property bool flipped: false
    property variant globalFindRecepie: findRecepieRect
    ...
    property int startingFlipppedWidth: flippedWidth

    Item {
        id: recepieDelegate

        //helper function to turn flippable element
        function flipStack()
        {
            if ( !findRecepieRect.flipped ) {
                recepieFlip.state = "flippedState"
                findRecepieRect.flipped = true
                foodCategory.z = 0

            }
        }

        // assign foods selected to an epicurious URL
        function getRecepieClicked()
        {
            var newUrl = "http://www.epicurious.com/tools/searchresults?search="
            var foodsProcessed = foodsInput.url
            var i
            do {
                i = foodsProcessed.indexOf(" ")
                if(i>0)
                    foodsProcessed = foodsProcessed.substring(0, i) + "+" + foodsProcessed.substring(i + 1, foodsProcessed.length);
                else if(i==0)
                    foodsProcessed = foodsProcessed.substring(i + 1, foodsProcessed.length);

            } while(i>=0)

            webBrowserContainer.urlString = newUrl+foodsProcessed            
        }
        ...
        Flipable {
            ...
            transform: Rotation {
                id: rotation
                axis {x:0; y:1; z:0}
                angle: recepieFlip.angle
            }

            // Front of the element has a Find Recepie Button
            front:
               Image {
                    id: imageFindRecepie
                    source: "pics/find-recipe/button-find-recipe-on.png"
                MouseArea {
                    anchors.fill: imageFindRecepie
                    onClicked: {
                        recepieDelegate.flipStack()
                        recepieDelegate.getRecepieClicked()
                    }
                }
            }

            // The back of the element has a Web Browser element
            back:
                WebBrowser {
                    id: webBrowserContainer
                    width: 480
            ...
            states: [
                ...
                State {
                    ...
                    PropertyChanges {
                        target: recepieFlip
                        x: epiRectangle.width - 110
                        y: epiRectangle.height - 35
                        rotation: 360
                    }
                    PropertyChanges {
                        target: findRecepieRect
                        x: 0
                        flipped: false
                    }
                },
