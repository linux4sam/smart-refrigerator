...
**
** Summary: Main QML file of the Epicurious application
**
****************************************************************************/
...
Rectangle {
    ...
    property alias mainRect: epiRectangle

    color: "#e5c000"

    // instantiate food picker element
    Rectangle {
        color: "#f2d84d"
        y:10
        anchors {left: parent.left; leftMargin: 25;}

        Image {
            id: foodImage
            source: "pics/images/foodCategory.png"

            MouseArea {
                id: mouseFoodPicker
                anchors.fill: parent
                onClicked: {
                    foodCategory.state = "showFoodPicker";
                }
            }
        }
    ...
    // foods to pick from model definition
    ListModel {
        id: buttonModel
        ListElement { name: "Chicken";  icon: "pics/images/have-chicken.png"; selected: false; }
        ListElement { name: "Eggs"; icon: "pics/images/have-eggs.png"; selected: false; }
        ListElement { name: "Fish"; icon: "pics/images/have-FISH.png"; selected: false; }
        ListElement { name: "Ham"; icon: "pics/images/have-ham.png"; selected: false; }
        ListElement { name: "Ketchup"; icon: "pics/images/have-ketchup.png"; selected: false; }
        ListElement { name: "Milk"; icon: "pics/images/have-milk.png"; selected: false; }
        ListElement { name: "Cheese"; icon: "pics/images/have-scheese.png"; selected: false; }
        ListElement { name: "Shrimp"; icon: "pics/images/have-shrimp.png"; selected: false; }
        ListElement { name: "Sushi"; icon: "pics/images/have-sushi.png"; selected: false; }

    }

    // Rectangle that holds a grid view with foods that user can add to the fridge
    Rectangle
    {
        id: rectangle1
        x: 145
        ...
        GridView {
            ...
            cellHeight: 60
            cellWidth: 100
            delegate: FoodPickButton {}
            model: buttonModel
        }

    }

    // The image element below allows user to pull up recepies based on fridge contents which is read from an external XML file
    Image {
        id: header
        width:parent.width
        ...
        property alias editUrl: foodsInput.url
        ...
        Item {
            ...
            {
                id: fridgeContents
                anchors {left: parent.left;leftMargin: 30; bottom: parent.bottom}

                    width: 40; height: 37
                    property bool selected: false;

                    Image {
                        id: fridgeContentsImage
                        width: 32
                        height: 37
                        source: "pics/icon-fridge.png"
                        smooth: false
                    }

                    MouseArea {
                        anchors.fill: fridgeContentsImage
                        onPressed: {
                            fridgeContents.state = "Pressed"
                            stateTimer.start()
                            ...
                            doc.onreadystatechange = function() {
                                if (doc.readyState === XMLHttpRequest.DONE) {
                                    ...
                                    // Go through first level children
                                    var level1 = root.childNodes[1];
                                    for (var i=0; i < level1.childNodes.length; ++i)
                                    {
                                        var level2 = level1.childNodes[i];
                                        for (var j=0; j < level2.childNodes.length; ++j)
                                        {
                                            if (level2.childNodes[j].nodeName === "name")
                                            {
                                                header.editUrl = header.editUrl + " " + level2.childNodes[j].childNodes[0].nodeValue
                                            }
                                        }
                                    }
                                }
                            ...
                            // The XML file must be available under assumed path
                            doc.open("GET", "../../../../webpage/fridge_contents.xml");
                            doc.send();
                        }
                    }
                    ...
                    Timer {
                        ...
                        interval: 200;
                        repeat: false
                        onTriggered: fridgeContents.state = 'State3'
                    }

                    states: [
                       State {
                            name: "Show"; when: myIcon.status == Image.Ready
                            PropertyChanges { target: fridgeContents; scale: 1 }
                        },

                        State {
                            name: "Pressed"
                            PropertyChanges { target: fridgeContents; scale: 0.7 }
                        },
                        State {
                            name: "State3"
                            PropertyChanges {target: fridgeContents; scale: 1.0 }
            ...
            // Next to the fridge contents button is the text field to enter foods for recepie
            FoodsInput {
                id: foodsInput
                anchors { left: fridgeContents.right; leftMargin: 0; right: parent.right; rightMargin: 15}
    ...
    // Base square and internal element provide implementation for the food category picker
    BaseSquare
    {
        id: baseSq
    ...
    // when a food category is picked, open a web browser
    Connections
    {
        target: foodCategory
        onCatSel: {
            baseSq.findRecp.webBrowser.urlString = category
            baseSq.findRecp.stackDeleg.flipStack()
        }
    }
