...
**
** Summary: Implements methods for manipulating XML file - read/write
**          These routines can get called from QML elements
**
****************************************************************************/
...
/***
 ** Default constructor
 ** @Parameters: QString file - name of the XML file to handle
 **/
XMLParser::XMLParser(QString file, QObject *parent) :
    QObject(parent)
{
    fileName = file;
...
/***
 ** This method removes food specified as parameter from the XML file
 ** @Parameters: QString foodName - name of the food to be removed
 **/
void XMLParser::removeFoodItem(const QString &foodName, int index)
{
    qDebug() << "Called the C++ method with" << foodName << " Index: " << index;
    QDomDocument *doc;
    doc = new QDomDocument("FoodList");
    QFile file(fileName);
    QDomElement root;
    ...
    QDomNodeList nodes = doc->elementsByTagName("food");

    //node to be removed
    QDomNode node = nodes.at(index);

    if(nodes.size() > 1)
    //second level node is my root
//    if(doc->documentElement().childNodes().at(0) != 0)
        root.childNodes().at(0).removeChild(node);

    //store new XML file in a QByteArray
    QByteArray xml = doc->toByteArray();

    //Write the new xml file back to the file
...
/***
 ** This method adds food specified by parameters to the XML file
 ** @Parameters:    QString foodName - name of the food to be removed
 **                 QString expirationDate - expiration Date
 **                 QString icon - icon for the food
 **                 QString quantityNum - number of units of a food
 **                 QString quantityUnit - unit name
 **/
void XMLParser::addFoodItem(const QString &foodName, const QString &expirationDate, const QString &icon, const QString &quantityNum, const QString &quantityUnit)
{
    qDebug("Added food");
    QDomDocument *doc;
    doc = new QDomDocument("FoodList");
    ...
    QDomElement child = newNode.firstChildElement("name");
    child.childNodes().at(0).setNodeValue(foodName);
    child = newNode.firstChildElement("icon");
    child.childNodes().at(0).setNodeValue(icon);
    child = newNode.firstChildElement("expiration");
    child.childNodes().at(0).setNodeValue(expirationDate);
    child = newNode.firstChildElement("quantityNum");
    child.childNodes().at(0).setNodeValue(quantityNum);
    child = newNode.firstChildElement("quantityUnit");
    child.childNodes().at(0).setNodeValue(quantityUnit);


    root.childNodes().at(0).appendChild(newNode);
