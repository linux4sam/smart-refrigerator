...
**
** Summary: View that visualizes foods currently in a fridge
**          Reads the FridgeContentsModel
**
****************************************************************************/
...
Rectangle {
    ...
    // Each food in the fridge needs to be represented by: icon, name, expiration date and close button
    Component {
        id: listFoodItem

        Rectangle {
            id: listFoodRect
            width: fridgeFoodList.width - margin
            height: 25
            color: "#a4034f"
            radius: 9
            x: margin
            ...
            Item{
                ...
                Image {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    anchors.right: parent.right
                    source: "qrc:///qml/smallFridgeContents/resources/buttons/button-close.png"
                }
                ...
                MouseArea {
                    ...
                    anchors.margins: -10
                    onClicked: {
                        cppXMLParser.removeFoodItem(foodName,index)
    ...
    // Creates list of foods in the fridge. foodsListModel created in main.qml
    ListView {
        id: fridgeFoodList
        spacing: 4
        // Initially no model is available
        // It is set dynamically according to user data
        model: foodsListModel
        delegate: listFoodItem
        width: parent.width - scroller.width
        height: parent.height
        focus: true
    }

    // Shows where you are if there is more foods that screen permits
    ScrollPosition {
        id: scroller
        x: fridgeFoodList.width
        position: fridgeFoodList.visibleArea.yPosition
        zoom: fridgeFoodList.visibleArea.heightRatio
        isScrollbarVisible: fridgeFoodList.moving
        height: listViewRect.height
    }
}
