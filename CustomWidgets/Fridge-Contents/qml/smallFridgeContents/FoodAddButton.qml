...
**
** Summary: Button that represents each food that can be dropped in the fridge
**
**
****************************************************************************/
...
import QtQuick 1.1
import "../../dragDrop.js" as ExternalJavaScript
...
Rectangle {
     ...
     property bool foodAdded: false;

     opacity: foodAdded ? 0.5 : 1.0

     property alias foodImage: myIcon.source
     property alias foodName: itemName.text

     // Each button has an icon and a name. Using column to fit both in a rectangle
     Column {
         id: buttonFood
     ...
     MouseArea {
         ...
         onPressed: {
             if(!disableRect){
                 ExternalJavaScript.windowWidth = 200;
                 ExternalJavaScript.startDrag(mouse);
             }
         }

         onPositionChanged: {
                 if(!disableRect)
                    ExternalJavaScript.continueDrag(mouse);
         }

         // When food is dragged and released on the fridge, activate Dialog Box to specify expiration date
         onReleased: {
             if(!disableRect)
             {
                 if(ExternalJavaScript.endDrag(mouse) === 1)
                 {
                     expDate.expFoodName = name
                     expDate.expIcon = icon
                     expDate.presetWindowData()
                     expDate.state = "visible"
                 }
             }
         }
     }
