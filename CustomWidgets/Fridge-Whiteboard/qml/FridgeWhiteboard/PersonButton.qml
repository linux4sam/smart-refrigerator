...
**
** Summary: This element implements person pick widget with all animations
**
****************************************************************************/
...
Item {
    ...
    Rectangle {
        ...
        height:120
        radius: width*0.5
        color: mainWhiteboard.selectedColor
        opacity: 0.5
    }
    ...
    Rectangle {
        ...
        color: mainWhiteboard.selectedColor
        opacity: 0.7
        anchors.centerIn: personPicker
    }

    // Person Picker circle
    Item{
        id: personPickerItem
        opacity: 1.0
        width:80
        height:80
        scale: 1.0
        anchors.centerIn: staticSurround

        Rectangle {
            id:personPickerButton
            width:80
            height:80
            radius: width*0.5
        ...
        // Stack all 4 people as defined in a model on the top of the widget
        PersonPickWidget {
            id: personPickerStack
            opacity: 1.0
            anchors.centerIn: parent
            anchors.fill: parent
    ...
    MouseArea {
        id:mousearea
        anchors.fill: personPicker
        onClicked: {
            personPicker.state = "pressed"
            stateTimer.start()
            personPickerStack.unroll()
        }
    }
    ...
    Timer {
        ...
        interval: 200;
        repeat: false
        onTriggered: personPicker.state = "default"
    }

    states: [
        State { name: "default" ;
                PropertyChanges { target: personPicker; scale:1.0}
        },
        State {
                name:  "pressed";
                PropertyChanges { target: personPicker; scale:1.3}
        }
    ]
    transitions: Transition {
        NumberAnimation { target: personPickerItem; property: "scale"; duration: 300;  easing.type: Easing.InOutQuart }
    }
}
