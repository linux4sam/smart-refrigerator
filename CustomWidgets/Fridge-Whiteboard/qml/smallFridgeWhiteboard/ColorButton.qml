...
Rectangle {
    ...
    // assign currently selected color to currently selected persona
    MouseArea {
        id:mousearea
        hoverEnabled:true
        anchors.fill: parent
        onClicked: {
            mainWhiteboard.selectedColor = colorPickerButton.color;
            personModel.personModel.setProperty(3, "currentColor", colorPickerButton.color)

            colorPickerButton.focus = true
            stateTimer.start()
            colorPickerTimer.restart()
        }
    }

    // Graphical part of the button itself
    Rectangle {
        visible: !focus
        x:3
        y:3
    ...
    Timer {
        ...
        interval: 1;
        repeat: false
        onTriggered: colorPickerButton.state = 'released'
    }

    states: [
        State { name: "focus" ;
                when: focus
                PropertyChanges { target: colorPickerButton; scale:1.0}
        },
        State { name: "released" ;
                PropertyChanges { target: colorPickerButton; scale:1.0}
        }
