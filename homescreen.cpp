    ...
    ui(new Ui::HomeScreen)
{
    ...
    connect (qmlView,SIGNAL(statusChanged(QDeclarativeView::Status)),this,
             SLOT(onViewStatusChanged(QDeclarativeView::Status)));

    if (isSmallResolution) {
        qmlView->setSource(QUrl::fromLocalFile("CustomWidgets/Fridge-MainScreen/qml/smallFridgeMainScreen/main.qml"));
    } else {
        qmlView->setSource(QUrl::fromLocalFile("CustomWidgets/Fridge-MainScreen/qml/FridgeMainScreen/main.qml"));
    }

    QObject *rootObject = dynamic_cast<QObject*>(qmlView->rootObject());
    connect(this, SIGNAL(setLoadingFlagQML(QVariant)), rootObject, SLOT(setLoadingFlag(QVariant)));
...
void HomeScreen::onViewStatusChanged(QDeclarativeView::Status status)
{
    if (status == QDeclarativeView::Ready)
    {
        connect ((QObject *)qmlView->rootObject(), SIGNAL(sigMainScreenFridgeZonesPressed()),
                 this, SLOT(onFridgeZonesPressed()));
        connect ((QObject *)qmlView->rootObject(), SIGNAL(sigMainScreenFridgeWhiteboardPressed()),
                 this, SLOT(onFridgeWhiteboardPressed()));
        connect ((QObject *)qmlView->rootObject(), SIGNAL(sigMainScreenEpicuriousPressed()),
                 this, SLOT(onFridgeEpiPressed()));
        connect ((QObject *)qmlView->rootObject(), SIGNAL(sigMainScreenWebBrowserPressed()),
                 this, SLOT(onFridgeWebBrowserPressed()));
        connect ((QObject *)qmlView->rootObject(), SIGNAL(sigMainScreenCalendarPressed()),
                 this, SLOT(onFridgeCalendarPressed()));
        connect ((QObject *)qmlView->rootObject(), SIGNAL(sigMainScreenContentsPressed()),
                 this, SLOT(onFridgeContentsPressed()));
    }
}

void HomeScreen::onFridgeZonesPressed()
{
    emit(fridgeZonesRequested());
}

void HomeScreen::onFridgeWhiteboardPressed()
{
    emit(fridgeWhiteboardRequested());
}

void HomeScreen::onFridgeEpiPressed()
{
    emit(fridgeEpiRequested());
}

void HomeScreen::onFridgeWebBrowserPressed()
{
    emit(fridgeBrowserRequested());
}

void HomeScreen::onFridgeCalendarPressed()
{
    emit(fridgeCalendarRequested());
}

void HomeScreen::onFridgeContentsPressed()
{
    emit(fridgeContentsRequested());
}

void HomeScreen::setLoadingFlag(bool flag)
{
    emit(setLoadingFlagQML(flag));
}
